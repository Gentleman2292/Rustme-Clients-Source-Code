...
extends GuiScreen {
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks) {
        ...
        if (this.anim >= 80.0f) {
            vec2fs.add(new Vec2f((float)x, (float)(y + 80)));
            vec2fs.add(new Vec2f((float)x + this.anim - 80.0f, (float)(y + 80)));
        } else {
            vec2fs.add(new Vec2f((float)x, (float)y + this.anim));
        }
        RenderUtils.drawSome(vec2fs, None.getColor().getRGB());
        vec2fs.clear();
        vec2fs.add(new Vec2f((float)(x + 20), (float)(y + 80)));
        if (this.anim >= 80.0f) {
            vec2fs.add(new Vec2f((float)(x + 20), (float)y));
            vec2fs.add(new Vec2f((float)(x + 20) - (this.anim - 80.0f), (float)y));
        } else {
            vec2fs.add(new Vec2f((float)(x + 20), (float)(y + 80) - this.anim));
        }
        RenderUtils.drawSome(vec2fs, None.getColor().getRGB());
    ...
    public void text(String test, int i, float size) {
        ...
        fr2.drawString(test, x + 10 - fr2.getStringWidth("N") / 2, y + fr2.getHeight() * i, i == 0 ? None.getColor().getRGB() : -1);
        GL11.glPopMatrix();
    }
