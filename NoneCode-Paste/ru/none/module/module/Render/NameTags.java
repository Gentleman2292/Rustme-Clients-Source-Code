...
extends Module {
    ...
    public void onRender3D() {
        ...
        for (Entity e : this.mc.world.loadedEntityList) {
            ...
            double x = e.lastTickPosX + (e.posX - e.lastTickPosX) * (double)this.mc.getRenderPartialTicks() - this.mc.getRenderManager().viewerPosX;
            double y = e.lastTickPosY + (e.posY - e.lastTickPosY) * (double)this.mc.getRenderPartialTicks() - this.mc.getRenderManager().viewerPosY;
            double z = e.lastTickPosZ + (e.posZ - e.lastTickPosZ) * (double)this.mc.getRenderPartialTicks() - this.mc.getRenderManager().viewerPosZ;
            GL11.glPushMatrix();
            GL11.glDisable((int)2929);
            GL11.glEnable((int)3042);
            GL11.glBlendFunc((int)770, (int)771);
            GL11.glNormal3f((float)0.0f, (float)1.0f, (float)0.0f);
            float size = Math.min(Math.max(1.2f * (this.mc.player.getDistanceToEntity(e) * 0.15f), 1.25f), 6.0f) * 0.015f;
    ...
    public int getcenter(String text) {
        return this.mc.fontRendererObj.getStringWidth(text) / 2;
    }

    public int getcenter(int text) {
        return this.mc.fontRendererObj.getStringWidth(String.valueOf(text)) / 2;
    }
}
