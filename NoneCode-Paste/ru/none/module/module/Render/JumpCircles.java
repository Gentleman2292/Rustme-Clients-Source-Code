...
extends Module {
    ...
    public void onUpdate(PlayerHook hook) {
        ...
        if (this.mc.player.motionY == 0.33319999363422365) {
            JumpCircles.handleEntityJump((Entity)this.mc.player);
        }
        JumpCircles.onLocalPlayerUpdate(this.mc.player);
    }

    @Override
    public void onRender3D() {
        super.onRender3D();
        EntityPlayerSP client = this.mc.player;
        double ix = -(client.lastTickPosX + (client.posX - client.lastTickPosX) * (double)this.mc.getRenderPartialTicks());
        double iy = -(client.lastTickPosY + (client.posY - client.lastTickPosY) * (double)this.mc.getRenderPartialTicks());
        double iz = -(client.lastTickPosZ + (client.posZ - client.lastTickPosZ) * (double)this.mc.getRenderPartialTicks());
        GL11.glPushMatrix();
