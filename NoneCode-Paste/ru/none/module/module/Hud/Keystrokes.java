    ...
    public Keystrokes() {
        ...
        for (KeyType keyType : KeyType.values()) {
            if (keyType == KeyType.UP) {
                this.keys.add(new key(() -> Keyboard.getKeyName((int)this.mc.gameSettings.keyBindForward.getKeyCode()), keyType, () -> this.getPosX() + 35, () -> this.getPosY(), this.mc.gameSettings.keyBindForward));
            }
            if (keyType == KeyType.DOWN) {
                this.keys.add(new key(() -> Keyboard.getKeyName((int)this.mc.gameSettings.keyBindBack.getKeyCode()), keyType, () -> this.getPosX() + 35, () -> this.getPosY() + 35, this.mc.gameSettings.keyBindBack));
            }
            if (keyType == KeyType.LEFT) {
                this.keys.add(new key(() -> Keyboard.getKeyName((int)this.mc.gameSettings.keyBindLeft.getKeyCode()), keyType, () -> this.getPosX(), () -> this.getPosY() + 35, this.mc.gameSettings.keyBindLeft));
            }
            if (keyType != KeyType.RIGHT) continue;
            this.keys.add(new key(() -> Keyboard.getKeyName((int)this.mc.gameSettings.keyBindRight.getKeyCode()), keyType, () -> this.getPosX() + 70, () -> this.getPosY() + 35, this.mc.gameSettings.keyBindRight));
        }
    }
