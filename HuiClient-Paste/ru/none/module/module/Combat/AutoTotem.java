...
extends Module {
    ...
    public void onUpdate(PlayerHook hook) {
        ...
        if (Minecraft.player.getHealth() <= this.health.getVal()) {
            if (Item.getIdFromItem((Item)Minecraft.player.inventoryContainer.getSlot(45).getStack().getItem()) != 449) {
                mc.getConnection().sendPacket((Packet)new CPacketEntityAction((Entity)Minecraft.player, CPacketEntityAction.Action.OPEN_INVENTORY));
                for (Slot slot : Minecraft.player.inventoryContainer.inventorySlots) {
                    ...
                    AutoTotem.mc.playerController.windowClick(Minecraft.player.inventoryContainer.windowId, slot.slotNumber, 0, ClickType.PICKUP, (EntityPlayer)Minecraft.player);
                    if (!this.timer.isDeley(MathUtils.getRandomInRange(500, 166))) continue;
                    AutoTotem.mc.playerController.windowClick(Minecraft.player.inventoryContainer.windowId, 45, 0, ClickType.PICKUP, (EntityPlayer)Minecraft.player);
                    TimerUtils.reset();
                }
            }
        }
