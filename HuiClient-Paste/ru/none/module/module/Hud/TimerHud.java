...
extends Module {
    ...
    public void onUpdate(PlayerHook hook) {
        ...
        if (this.timer.isDeley(5L) && None.moduleManager.getModule("Timer").isEnable() && b) {
            TimerUtils.reset();
            this.level = (int)((float)this.level - TimerHud.mc.timer.field_194147_b * 4.0f);
            if (this.level <= 0) {
                this.level = 0;
            }
        }
        if (!b && this.timer.isDeley(1009L) && None.moduleManager.getModule("Timer").isEnable() || this.timer.isDeley(1009L) && !None.moduleManager.getModule("Timer").isEnable()) {
            TimerUtils.reset();
